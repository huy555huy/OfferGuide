...
def _exec_search_official_jobs(args: dict[str, Any], deps: AgentRuntimeDeps) -> str:
...
def _exec_score_match(args: dict[str, Any], deps: AgentRuntimeDeps) -> str:
...
def _exec_tailor_advice(args: dict[str, Any], deps: AgentRuntimeDeps) -> str:
    if deps.runtime is None:
...
def _exec_interview_prep(args: dict[str, Any], deps: AgentRuntimeDeps) -> str:
    if deps.runtime is None:
...
def _exec_reflect_outcome(args: dict[str, Any], deps: AgentRuntimeDeps) -> str:
    if deps.runtime is None:
...
def _exec_schedule_next_wake(args: dict[str, Any], deps: AgentRuntimeDeps) -> str:
...
def _exec_detect_evolution_candidates(args: dict[str, Any], deps: AgentRuntimeDeps) -> str:
...
def _exec_evolve_skill(args: dict[str, Any], deps: AgentRuntimeDeps) -> str:
    if deps.llm is None:
...
def _exec_run_release_cycle(args: dict[str, Any], deps: AgentRuntimeDeps) -> str:
...
def _record_event_row(
    deps: AgentRuntimeDeps, *, kind: str, job_id: int | None, note: str,
) -> int:
    from . import _schema
