...
def _fetch_and_ingest(
    raw: str, deps: AgentRuntimeDeps, *, company_hint: str, title_hint: str,
) -> tuple[int, str | None]:
...
def _invoke_skill(
    deps: AgentRuntimeDeps, spec: Any, *, inputs: dict[str, Any],
) -> SkillResult | None:
...
def _load_job_meta(deps: AgentRuntimeDeps, job_id: int) -> dict[str, Any] | None:
