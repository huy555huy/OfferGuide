...
W14 → W15 boundaries clear and lets us evolve harness internals without
churning the central DB module.
"""
...
# Per-run record: every harness loop run gets a row. Used for cost
...
def init_harness_schema(store: Store) -> None:
    """Idempotent: create harness tables if missing.

    Call this once at harness startup (before first run).
    """
    with store.connect() as conn:
        for ddl in _ALL_SCHEMAS:
            conn.executescript(ddl)
    log.debug("harness schema initialized (3 tables)")
